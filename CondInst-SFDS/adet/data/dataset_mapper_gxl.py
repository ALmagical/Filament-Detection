...
# @gxl
from detectron2.data.transforms.augmentation_impl import RandomRotation, RandomBrightness, RandomContrast, RandomFlip
...
class DatasetMapper_GXL(DatasetMapper):
    """
    ...
    def __init__(self, cfg, is_train=True):
        ...
        # Add online data augmentations
        # @gxl
        # TODO Try to design a offline function to do same thing.
        
        self.augmentation.insert(0, 
            RandomRotation(angle=[0, 40], expand=False))
        
        self.augmentation.insert(0, 
            RandomContrast(intensity_min=0.6, intensity_max=1.3))
        self.augmentation.insert(0, 
            RandomBrightness(intensity_min=0.6, intensity_max=1.3))
        self.augmentation.insert(0, 
            RandomFlip(prob=0.5, horizontal=True, vertical=False))
        self.augmentation.insert(0, 
            RandomFlip(prob=0.5, horizontal=False, vertical=True))
        
        logger.info("Augmentations used in training: " + str(self.augmentation))
