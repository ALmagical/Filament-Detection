    ...
    def __init__(
        ...
        *,
        ...
        if self.use_attention == True:
            ...
            #self.attention = ParallelPolarizedSelfAttention(channel=out_channels)
...
class Focus_my(CNNBlockBase):
    ...
    def __init__(self,  in_channels=3, out_channels=64, norm="BN", activation="Mish"):
        ...
        if out_channels <= 64:
            internel_channels = out_channels
        else:
...
def make_stage(*args, **kwargs):
    ...
    """
    return ResNetD_GXL.make_stage(*args, **kwargs)


@BACKBONE_REGISTRY.register()
def build_resnetd_se_backbone(cfg, input_shape):
    """
    ...
    for idx, stage_idx in enumerate(range(2, max_stage_idx + 1)):
        ...
        blocks = ResNetD_GXL.make_stage(**stage_kargs)
        in_channels = out_channels
        out_channels *= 2
        bottleneck_channels *= 2
        stages.append(blocks)
    return ResNetD_GXL(stem, stages, out_features=out_features).freeze(freeze_at)