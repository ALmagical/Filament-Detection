...
def build_fcos_resnet_fpn_backbone(cfg, input_shape: ShapeSpec):
    ...
    elif cfg.MODEL.RESNETD:
        bottom_up = build_resnetd_backbone(cfg, input_shape)
    elif cfg.MODEL.RESNETD_SE:
        bottom_up = build_resnetd_se_backbone(cfg, input_shape)
    elif cfg.MODEL.RESNETD_PPSA:
        bottom_up = build_resnetd_ppsa_backbone(cfg, input_shape)
    else:
    ...
    if top_levels == 1:
        ...
        top_block = None
    if cfg.MODEL.FPN.ATTENTION == None:
        backbone = FPN(
            bottom_up=bottom_up,
        ...
        backbone = FPN_ATTENTION(
            ...
            top_block=top_block,
            fuse_type=cfg.MODEL.FPN.FUSE_TYPE,
            attention=cfg.MODEL.FPN.ATTENTION,
        )
    return backbone
