...
from adet.layers import ml_nms, IOULoss, ml_nms_gxl, mc_nms
# @gxl Tensorboard
...
class FCOSOutputs(nn.Module):
    def __init__(self, cfg):
        ...
        self.radius = cfg.MODEL.FCOS.POS_RADIUS # 1.5
        ...
        self.loc_loss_func = IOULoss(cfg.MODEL.FCOS.LOC_LOSS_TYPE) # giou
        ...
        self.thresh_with_ctr = cfg.MODEL.FCOS.THRESH_WITH_CTR # True

        self.num_classes = cfg.MODEL.FCOS.NUM_CLASSES # 3
        self.strides = cfg.MODEL.FCOS.FPN_STRIDES # [8, 16, 32, 64, 128]

        # @gxl
        self.special_classes = cfg.MODEL.CONDINST.MASK_HEAD.SPECIAL_CLASSES
        self.special_classes_thresh = cfg.MODEL.CONDINST.MASK_HEAD.SPECIAL_CLASSES_THRESH
        self.nms_thresh_nom = cfg.MODEL.CONDINST.MASK_HEAD.NMS_THRESH_NORM
        self.nms_thresh_spc = cfg.MODEL.CONDINST.MASK_HEAD.NMS_THRESH_SPC
        self.nms_thresh_all = cfg.MODEL.CONDINST.MASK_HEAD.NMS_THRESH_ALL
        self.use_centroid = cfg.MODEL.CONDINST.USE_CENTROID
        ...
        soi = []
        prev_size = -1
        for s in cfg.MODEL.FCOS.SIZES_OF_INTEREST: # [64, 128, 256, 512]
    ...
    def _get_ground_truth(self, locations, gt_instances):
        # Compute the number of point at every loction level
        # 计算每个层级的坐标数量
        num_loc_list = [len(loc) for loc in locations]

        # compute locations to size ranges
        # 生成与locations对应的list，数值为sizes_of_insterest
        loc_to_size_range = []
    ...
    def compute_targets_for_locations(self, locations, targets, size_ranges, num_loc_list):
        ...
        for im_i in range(len(targets)):
            ...
            if self.center_sample:
                ...
                # torch.min(dim) 返回在指定维度上的最小值及其索引
    ...
    def losses(self, logits_pred, reg_pred, ctrness_pred, locations, gt_instances, top_feats=None):
        ...
        if self.debug:
            ...
            for i in range(len(logits_pred)):
                put_image_to_tensorboard(logits_pred[i], storage, 'logits_pred_' + str(i+1))
                # put_image_to_tensorboard(reg_pred[i], storage, 'reg_pred_' + str(i+1))
                put_image_to_tensorboard(ctrness_pred[i], storage, 'ctrness_pred_' + str(i+1))
        ...
        if len(top_feats) > 0:
            ...
            if self.debug:
                for i in range(len(top_feats)):
                    put_image_to_tensorboard(top_feats[i], storage, 'top_feat_' + str(i+1))
    ...
    def fcos_losses(self, instances):
        ...
        # @gxl
        # pos_inds means the indexes of positive samples in the gt
        # So when training, FCOS get the positive samples from the gt
    ...
    def predict_proposals(
        ...
        # @gxl
        # When training, the program wouldn't go to there.
        # So why set this 'if'
        if self.training:
    ...
    def forward_for_single_feature_map(
        ...
        # @gxl
        # Candidate inds means the pixels in the feature map which source
        # are greater than the thresh.
        candidate_inds = logits_pred > self.pre_nms_thresh
        pre_nms_top_n = candidate_inds.reshape(N, -1).sum(1)
        # @gxl
        # If the number of predictions are more than pre_nms_topk, it will be crop.
